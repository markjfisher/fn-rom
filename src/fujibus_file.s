...
;
;   Base URI may overlap fuji_data_buffer (e.g. fuji_current_fs_uri in same region).
...
;         ; Copy base URI to fuji_data_buffer+32 so it cannot overlap payload at +6..24
;         ldy     #$00
; @precopy_base:
;         cpy     aws_tmp02
;         beq     @precopy_done
;         lda     (aws_tmp00),y
;         sta     fuji_data_buffer+32,y
;         iny
;         bne     @precopy_base
; @precopy_done:
;         lda     #<(fuji_data_buffer+32)
;         sta     aws_tmp00
;         lda     #>(fuji_data_buffer+32)
;         sta     aws_tmp01

;         lda     #FN_PROTOCOL_VERSION
;         sta     fuji_data_buffer+FN_HEADER_SIZE+0

;         lda     aws_tmp02
;         sta     fuji_data_buffer+FN_HEADER_SIZE+1
;         lda     #$00
;         sta     fuji_data_buffer+FN_HEADER_SIZE+2
...
;         beq     @copy_arg_len
;         lda     (aws_tmp00),y
;         sta     fuji_data_buffer+FN_HEADER_SIZE+3,y
;         iny
;         bne     @copy_base
...
;         ; We built the payload at fuji_data_buffer+FN_HEADER_SIZE; pass that pointer so
;         ; the copy does not overwrite our data (and so we don't copy from base URI).
;         lda     #<(fuji_data_buffer+FN_HEADER_SIZE)
;         sta     aws_tmp00
;         lda     #>(fuji_data_buffer+FN_HEADER_SIZE)
;         sta     aws_tmp01
;         ldy     aws_tmp06
...
;         lda     fuji_data_buffer+FN_PARAMS_OFFSET
;         bne     @error

;         lda     fuji_data_buffer+FN_HEADER_SIZE+0
;         cmp     #FN_PROTOCOL_VERSION
;         bne     @error

;         ; flags: bit0=isDir, bit1=exists. Path must exist on target (FujiNet has validated via stat).
;         lda     fuji_data_buffer+FN_HEADER_SIZE+1
...
; ; Output:
; ;   response packet left in fuji_data_buffer for caller to print/consume
; ;   C clear on success, set on failure
