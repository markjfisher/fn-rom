...
;
; We build the payload directly in fuji_data_buffer after the 6-byte FujiBus header,
...
;         ldx     #$00
; @copy_uri:
;         cpx     fuji_data_buffer+FN_HEADER_SIZE+2
;         beq     @write_mode
;         lda     fuji_current_fs_uri,x
;         sta     fuji_data_buffer+FN_HEADER_SIZE+3,x
;         inx
;         bne     @copy_uri
...
;         lda     #$01                    ; mode len = 1
;         sta     fuji_data_buffer+FN_HEADER_SIZE,y
;         iny
;         lda     #'r'
;         sta     fuji_data_buffer+FN_HEADER_SIZE,y
;         iny
...
;         ; fn_build_packet copies payload from (aws_tmp00); point at fuji_data_buffer payload.
...
;         sta     fuji_data_buffer+FN_HEADER_SIZE+1
;         sta     fuji_data_buffer+FN_HEADER_SIZE+2
;         sta     fuji_data_buffer+FN_HEADER_SIZE+3
...
; ;   fuji_current_mount_slot = 0-based slot index
; ; Output:
; ;   response remains in fuji_data_buffer
; ;   Carry clear on success, set on failure
; ;
...
;         ; On success the FujiDevice response record is left in fuji_data_buffer for
