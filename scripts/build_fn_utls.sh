...
SSD="${FN_UTLS_SSD:-$root/build/FN-UTLS.ssd}"
...
cat > "$UTILCFG" <<EOF
# Generated by scripts/build_fn_utls.sh for BUILD_MACHINE=$MACHINE.
MEMORY {
...
with open(out, "w") as f:
    f.write("; Generated for FN-UTLS transient utility linkage.\n")
...
if [ -n "${FN_UTLS_EXTRA_STAGE:-}" ]; then
  if [ ! -d "$FN_UTLS_EXTRA_STAGE" ]; then
    echo "FN_UTLS_EXTRA_STAGE is not a directory: $FN_UTLS_EXTRA_STAGE" >&2
    exit 1
  fi
  echo "==> staging extra utility files from $FN_UTLS_EXTRA_STAGE"
  cp -R "$FN_UTLS_EXTRA_STAGE"/. "$STAGE"/
fi
