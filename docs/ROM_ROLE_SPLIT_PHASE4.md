...
`boot-bin/!BOOT` restores the configured boot image to drive 3 and sets it as the library, so an unrecognised
...
1. **rom_abi from the ROM .lbl.** `scripts/build_fn_boot.sh` builds the `UTILITIES=disk` ROM, then
...
2. **RAM binaries.** Each command is assembled for `$1900` (`cfg/fn-util.cfg`) with `FN_UTIL_BINARY`
   ...
   are linked in from their `src/utils/` source per §5.5. A tiny entry wrapper (`boot-bin/<cmd>.s`)
...
3. **FN-BOOT.ssd.** `create_ssd.py` bundles the binaries (per-file `.inf` load/exec `$1900`) + `!BOOT`.
