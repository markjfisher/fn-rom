...
Our plan in this task is to update the b2-rom project to be compatible with the fujinet-nio fujibus transport.

b2-rom is a project to create a bbc 8 bit ROM that can be burned onto eeprom and placed in the bbc micro (or loaded into the emulator) to act as a bridge between the bbc micro, and the fujinet-nio device for network based disk images.
...
Either way, the b2-rom project connects the 2 sides together to allow network commands that allow disk drive behaviour over a network using files for the images instead of physical floppy disks.
That is where b2-rom will end for functionality, i.e. purely for disk interaction via fujinet - it is similar to the MMFS project allowing SD cards containing disk images to be connected to a BBC and act as disks to the BBC's OS. In fact b2-rom is based on MMFS from a code point of view, it sets up the "*" commands that a BBC uses for interacting with ROMs to be able to list files, boot disks, read files, execute files etc.

The b2-rom project has an architecture described in @/../../bbc/fn-rom/docs/ARCHITECTURE.md 

We need to update the b2-rom from using the legacy transport for communicating with the legacy fujinet-firmware to instead using fujibus protocol for the fujinet-nio rewrite of fujinet-firmware.
...
- Update the b2-rom project to support fujibus (this does not have to live in fuji_serial.s directly) in 6502 BBC ASM (there is a ton of code already in b2-rom for that pattern)
