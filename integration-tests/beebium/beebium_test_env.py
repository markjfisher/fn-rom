"""Environment resolution for fn-rom Beebium integration tests.
...
Everything else is derived automatically. Set any of the usual override variables
(BEEBIUM_SERVER, BEEBIUM_MOS, FUJINET_TOOLS, …) only when autodetection is wrong
for your machine.
...
def _exit(message: str) -> None:
    pytest.exit(
        f"{message}\n"
        f"Required: BEEBIUM_HOME and FUJINET_NIO_HOME. "
...
def resolve_beebium_basic(home: Path) -> Path | None:
...
def resolve_fujinet_tools_root() -> Path:
    """Directory to put on sys.path (the parent of the fujinet_tools package)."""
    ...
    if explicit is not None:
        ...
        if (explicit / "fujinet_tools").is_dir():
            return explicit
        pkg_init = explicit / "__init__.py"
        if explicit.name == "fujinet_tools" or (
            pkg_init.is_file() and explicit.parent.name == "py"
        ):
            return explicit.parent
        if explicit.is_dir():
            return explicit
    ...
    if (py_root / "fujinet_tools").is_dir():
        return py_root
    _exit(
        f"fujinet_tools not found under {py_root}; set FUJINET_TOOLS to fujinet-nio/py"
    )


def ensure_environment() -> None:
    """Resolve paths once and publish them back into os.environ."""
    global _CONFIGURED
    if _CONFIGURED:
        return
    ...
    basic = resolve_beebium_basic(home)
    if basic is not None:
        os.environ.setdefault("BEEBIUM_BASIC", str(basic))
    os.environ.setdefault("FUJINET_NIO_HOME", str(nio))
    # Always normalize — users often point FUJINET_TOOLS at .../py/fujinet_tools.
    os.environ["FUJINET_TOOLS"] = str(resolve_fujinet_tools_root())

    services = nio / "scripts" / "start_test_services.sh"
    if not services.is_file():
        _exit(f"Missing {services}")
