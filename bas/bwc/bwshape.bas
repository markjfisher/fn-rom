REM filename: BWSHAPE
REM Fetch bounce-world shapes and print BBC BASIC DATA lines
...
PRINT "Bounce World Shape Export"
PRINT "========================="
PRINT
...
PRINT
PROCemit_shape_data(shapeCount%)
...
totalRead%=FNtcp_read_min(handle%, responseBuffer, 0, 1, RESPONSE_BUFFER_SIZE%, RD_RETRY_LIMIT%)
...
REPEAT
  ...
  IF FNhave_complete_shape_payload(shapeCount%, totalRead%)=FALSE THEN read%=FNtcp_read_min(handle%, responseBuffer, totalRead%, 1, RESPONSE_BUFFER_SIZE%-totalRead%, 10)
...
UNTIL tries%>=RD_RETRY_LIMIT% OR totalRead%>=RESPONSE_BUFFER_SIZE% OR FNhave_complete_shape_payload(shapeCount%, totalRead%)
...
DEF PROCemit_shape_data(shapeCount%)
LOCAL shapeIndex%, width%, dataLen%, srcOffset%
PRINT "REM Generated shape data"
PRINT "shapeCount%=";shapeCount%
FOR shapeIndex%=0 TO shapeCount%-1
  width%=shapeWidth%(shapeIndex%)
  dataLen%=width%*width%
  srcOffset%=shapeOffset%(shapeIndex%)
  PRINT "DATA ";shapeId%(shapeIndex%);",";width%;",";dataLen%;",";FNshape_string$(srcOffset%, dataLen%)
NEXT
ENDPROC

DEF FNshape_string$(srcOffset%, dataLen%)
LOCAL i%, out$, ch%
out$=CHR$(34)
FOR i%=0 TO dataLen%-1
  ch%=responseBuffer?(srcOffset%+i%)
  out$=out$+FNconvert_emit_char$(ch%)
NEXT
out$=out$+CHR$(34)
=out$

DEF FNconvert_emit_char$(ch%)
IF ch%=92 THEN ="\\"
IF ch%=34 THEN ="'"
IF ch%=114 OR ch%=41 OR ch%=76 OR ch%=33 OR ch%=74 OR ch%=116 OR ch%=84 OR ch%=50 OR ch%=43 THEN ="+"
IF ch%=97 OR ch%=98 OR ch%=99 OR ch%=100 OR ch%=105 OR ch%=106 OR ch%=107 OR ch%=108 OR ch%=109 THEN ="#"
IF ch%=101 OR ch%=102 OR ch%=103 OR ch%=104 THEN ="."
IF ch%>=32 AND ch%<127 THEN =CHR$(ch%)
="?"
