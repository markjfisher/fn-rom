...
Copy-paste **Cursor / Claude MCP and agent snippets** live under **`soft65c02_mcp/agents/`** (see **`README.md`** there).
...
Drop every symbol from the current table (parser context and session state both update):

```
symbols clear
```

If no table exists yet, the command still succeeds with a short message.

#### symbols remove

Remove **every** symbol whose **name** matches a [Rust regex](https://docs.rs/regex/latest/regex/#syntax). Pattern is either a **quoted** string (same escapes as elsewhere) or a **bare** `symbol_name`, which is treated as an **exact** match (`^` + `regex::escape(name)` + `$`).

```
symbols remove "__.*"              // e.g. cc65 segment labels .__BSS_LOAD__, .__MAIN_START__, …
symbols remove foo                 // same as "^foo$" — only the symbol named `foo`
```

Invalid regex: parse error (line skipped in `-c` mode). No symbols matched: success with a note. Requires a symbol table for removals to occur (otherwise a short message).
