...
### Symbol Dump / Grep / Clear
```
symbols dump                            // All symbols ($addr  name); always printed
symbols grep "fujinet.*"                // Rust regex on names; `(no matches)` if empty
symbols clear                           // Drop entire symbol table
```

### Symbol Remove (regex; bare name = exact `^name$`)
```
symbols remove "__.*"                   // Quoted Rust regex on symbol names
symbols remove foo                      // Exact symbol `foo` only
```
